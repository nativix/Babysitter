...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    let darkPink = UIColor(netHex:0xE89789)
    let lightPink = UIColor(netHex:0xf7c7b9)
    let white = UIColor.whiteColor()

    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        // Override point for customization after application launch.
        
        UINavigationBar.appearance().barTintColor = darkPink
        UINavigationBar.appearance().titleTextAttributes = [NSForegroundColorAttributeName: white]
        UITabBar.appearance().barTintColor = darkPink
        UITabBarItem.appearance().setTitleTextAttributes([NSForegroundColorAttributeName: lightPink], forState:.Normal)
